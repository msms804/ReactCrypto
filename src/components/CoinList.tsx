                    ...
                    <tbody>
                        {selectedCurrency === "KRW" ?
                            filteredKrwCoins?.slice(0, 20).map((item, index) => (
                                <tr key={index}>
                                    <td onClick={() => { onClickWatched(item.shortname) }}>
                                        {watchList.includes(item.shortname)
                                            ? <svg xmlns="http://www.w3.org/2000/svg" fill="orange" viewBox="0 0 24 24" strokeWidth="1.5" stroke="none" className="size-6 w-4 h-4">
